...
# ╔═╡ c3f57538-83aa-4d78-a220-ddf913dd00c9
@bind l3 Slider(-π:0.005:π,  default=0.005)
#Asignamos un valor a 'l3' a través de un deslizador interactivo que toma valores en -π:0.005:π

# ╔═╡ a6fb120b-685b-4223-863d-eac33cc5713d
l3 #Asignamos valor a 'l3'

# ╔═╡ 59836adc-c12b-4ab4-8d6f-f9a2dc1e059e
begin
	plot(sin, -π:l3:π, title = "Funciones trigonométricas", xlabel = "x", ylabel = "y", color = "darkgreen", label = "sin") #Haz una gráfica plot para sin 
    plot!(cos, -π:l3:π, color = "blue", label = "cos") #Haz una gráfica plot para cos
end
...
begin
	...
	anim10 = @animate for t in 0:0.1:2π # Por cada t ∈ 0:0.1:2π,
	plot(f.( range(0,2π, step = 0.1) .- t/B),      #grafica Asin(B(x-t/B))+C para todas las x en
		 legend = false, title = L"A\sin(Bx-t)+C") #el rango de 0 a 2π con tamaño de paso 0.1.
	end

	gif(anim10, "ondaEnMovimiento.gif", fps = 30)
...
x(t)=r*cos(θ)*t
y(t)=((h+r)*sin(θ)*t)-((9.81*(t^2))/2)

@gif for t in 0:1:20
	scatter(y.( range (0,20, step =1)), legend= false, title= "Tiro parabólico")
	end
end

# ╔═╡ 97118b24-0ca7-47f3-8dee-d949d67fe1d9
md"""**Ejercicio** Crea una función que tome parámetros h, r, θ, t, y haga lo descrito en el ejercicio anterior."""

# ╔═╡ a631e550-9778-4521-ae60-81b1b372c3d2
t = linspace(0,1.8)
cond_inicial = array([0,5,0,sqrt(100-5**2)]) # el modulo de la velocidad es 10
solucion = odeint(sist_ec_dif_fric,cond_inicial,t,args=(0.03,15)) # Con friccion
plot(solucion[:,0],solucion[:,2],label="Con fricción y $g$ modificada")
solucion = odeint(sist_ec_dif,cond_inicial,t) # Sin friccion
plot(solucion[:,0],solucion[:,2],'o-',label="Sin fricción")
legend(),grid()
xlim([0,10]),ylim([0,4])
show()
...
# ╔═╡ 8650e71b-6b60-4acf-950d-595976cdf18a
begin
	W=(1:0.1:2π)
	U=(1:0.1:2π)
	Z(x,y)=cos(x)+sin(y) 
...
# ╔═╡ cb9bfa91-e077-468b-897c-fcecee8c51f3

@gif for t in (1:0.1:2π,
	surfacee(W,U,Z.(range 0,2π, step=0.1),legend= false, title = "Ejercicio"))
