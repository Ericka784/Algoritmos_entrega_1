...
# ╔═╡ 31bd51d1-a7c9-455d-801f-6ca565064c89
 using Plots, PlutoUI
...
function newton(f, f′, x0, n)            #Definimos la funcion
		sol = x0 - (f(x0)/f′(x0))         #Colocamos lo que queremos que nos regrese como solucion que es la aproximacion de la raiz.
		for i = 1:n                      #Colocamos el ciclo for para indicar la secuencia n de las operaciones.
			sol = sol - (f(sol)/f′(sol)) #Sustituimos en la formula de la aproximacion.

		end                              

        print(sol)              	     #Imprimimos la función que nos dara el resultado
end

	
         

# ╔═╡ 34c7aaa7-33bd-4fe7-9e51-8f7e2cca46a2

begin 
  h(x)= x^2+4x #colocamos una funcion y su derivad
	h′(x)= 2x+4
end

# ╔═╡ 0b5f02f3-56d6-446a-8749-3ed707b14c5b
newton(h,h′,4,2) #Imprimimos la función
...
function newtonRecursiva(f :: Function, f′, x1::Number, m) #Definimos la función 
   sol_2= x1 - (f(x1)/f′(x1))     #colocamos la definicion de la aproximacion de la raiz.

	if m == 0  #Realizamos un ciclo if else 
		println(sol_2)
		
	else m > 0
		for i = 1:m
			sol_2 = sol_2 - (f(sol_2)/f′(sol_2)) 
...
# ╔═╡ 30c25d39-c7cb-4ddc-996d-5bcbc0372f93

begin 
  k(x)= cos(x) # XColocamos una función y su derivada

	k′(x)= -sin(x)
end

# ╔═╡ a7c1b770-7b88-4265-a180-0fab143aff38
newtonRecursiva(k,k′, 3, 5) #Imprimimos su resultado
...
function newtonDistancia(f, f′, x2, ε)  #Definimos la función
	

	if ε ≤ 0               #iniciamos un ciclo if esto para poder colocar el valor de ε, debido a que nos pide que al momento en que se realicen distintas iteraciones y que cuando dos se aproximen sien ε menor a cero se marque un mensaje de error
		print("Error")

		#cuando la aproximacion sea mayor a ε, siendo x2+ε, devido que queremos una aproximacion positiva.

	else ε > 0 
	 sol_3 =  x2+ε- ((f(x2+ε)/f′(x2+ε)))     # cuando ε sea mas grande se se imprima nustra funcion de aproximacion de raiz.
...
# ╔═╡ 7f03e407-cbe1-4dde-b345-b863338d540a
begin 
  r(x)= cos(x)
...
# ╔═╡ 0f3601d7-7d8d-4531-8d04-c8956bc577b2
newtonDistancia(r, r′, 5, 0)

# ╔═╡ 1e37172d-e063-45c0-a376-5fb963f499bd
newtonDistancia(r, r′, 5, 7) #Imprimimos la función
...
function derivadaNumerica(fd :: Function, x3:: Real , h:: Number) #definimos la funcion 
	der_1= (h*(fd(x3+1/h)-fd(x3)))    #Colocamos que queremos que nos regrese la aproximacion de la derivada.
	println(der_1)
end


# ╔═╡ ef934ba9-9d7c-4625-a99f-c254e5ba879d
 begin
	p(x)= 2x^2 #Colocamos la función 
 end

# ╔═╡ 48893540-6345-4220-8296-e3a0a7404669
derivadaNumerica(p, 10, 10*100000) #Imprimimos la función
...
function newtonDN(f, x4, h , n :: Number) #Definimos la función 
	der_2= (h*(f(x4+1/h)-f(x4)))
	sol_4(x4)= sol_4 - (f(sol_4)/der_2(sol_4))    #Como segundo paso tenemos que poner la aproximacion de raiz .    

	if n ≥ 0 
		for i = 1:n                      #Colocamos el ciclo for para indicar la secuencia n de las operaciones.
			sol_4(x4) = sol_4 - (f(sol_4)/der_2(sol_4)) #Sustituimos en la formula de la aproximacion.
...
# ╔═╡ a565ca9f-39da-4f0a-abe1-65d442fbbdc2
begin 
 j(x)= x^2 #Colocamos la función
end

# ╔═╡ a151681e-bad1-4496-911c-6f9a470bb65a
newtonDN(j, 10, 1010000, 10) #Imprimimos la función
...
fd2 = cos # Luego, usa esta celda para reasignar `f`
...
# ╔═╡ b4435824-88dc-41d0-b31b-101e6e150364
@bind hd Slider(1000:1000:1000000, default=10000000) #COLOCAMOS LOS RANGO DE CADA PARAMETRO Y ASI YA DEFINIMOS LA FUNCION.

# ╔═╡ f8b8b2bb-44f4-42e3-98fc-01f424754a0d
@bind xf Slider(1:1000, default=1000)

# ╔═╡ 7d861787-0853-4780-8d1f-43c2bd2b5d11
@bind n Slider(1:1000, default=1000) 

# ╔═╡ 45a75037-600f-48cc-b56e-0dbc3642e447
x5 = collect(-100:0.8:xf)

# ╔═╡ 731751a3-8ede-4031-9076-4df4cedaffef

	ffin(x5)= fd2(x5)


# ╔═╡ 2858d6be-d3c3-4592-b0c2-511cd59cb870
begin
plot((x5,ffin), label= "cos" ) # Pedimos que se imprima la grafica 
	
scatter!((x5,ffin), xlims=(-15.7,15.7), ylims=(-1.0,1.0), xlabel = "xf",  color = ["purple" colorant"#00ff00"], ylabel = "fd2", label= "aprox") #Graficamos 
end
