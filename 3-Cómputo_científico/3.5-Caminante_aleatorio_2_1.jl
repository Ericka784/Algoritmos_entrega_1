...
# ╔═╡ 032f237e-0650-43a2-a3c2-dbb9ab9e25ac
using Plots
...
# ╔═╡ 6fd453a4-934a-4a5b-b223-273964812ecd
rand()

# ╔═╡ 602e852a-5a9b-46b4-bc36-95e53f03c062
rand(Int)

# ╔═╡ d2ee2460-9e91-420c-adb3-085b635547d7
rand(Bool)
...
# ╔═╡ 6cc42fe8-aad4-47c7-a74b-9a3ae742e58b
rand(n)

# ╔═╡ 02cba942-eb6d-414b-a9a5-01619601beb1
rand(Int, n)

# ╔═╡ 95bfe30a-c7f3-4fb9-b683-ca9e023b0127
rand(Bool, n)
...
# Definimos el intervalo
	a=5 # Definimos el limite inferior 
	b=10 #Definimos el limite superior 
	
end

# ╔═╡ be8fb198-5dba-4b9e-a377-b0f94e86b6e0

function aleatorioUniforme(a, b) # Definimos la función (tomando de                                           argumento a y b)
    return a + (b - a) * rand()   #  Utilizamos (b - a) para obtener la amplitud del intervalo y lo sumamos a a para asegurarnos de que el número generado esté dentro del rango correcto, luego lo multiplicamos por rand() para generar un número aleatorio 
end

# ╔═╡ 57d04997-a2dd-4e47-a0f2-b26a9f13c7e0
aleatorioUniforme(a,b) #Imprimimos la función 
...
# ╔═╡ 4a16af8c-09f7-40bb-946b-4b22a619aaf9
begin
	
	num_samples = 10000  # Definimos la cantidad de muestras a utilizar 
	
	function verificarUniformidad(a, b, num_samples) #  Definimos la                       función (tomando de argumento a, b  y num_samples)

		
	    # Generamos muestras aleatorias
	    muestras = [aleatorioUniforme(a, b) for _ in 1:num_samples] 
	
	    # Creamos el histograma
	    histogram(muestras, bins = 5, xlabel = "Valor", ylabel = "Frecuencia", title = "Distribución uniforme entre a y b", legend = false)
	end

end

# ╔═╡ f8868c56-29da-472b-9069-7368c8804d4a
	verificarUniformidad(a, b, num_samples) # Imprimimos la función que nos da como resultado el histograma
...
function unPaso() # Definimos la función 
    return 2 * Int(rand(Bool)) - 1

	# Usamos rand(Bool) para generar un valor booleano aleatorio, y luego se convierte en un entero utilizando Int Luego, se multiplica por 2 y se resta 1 para obtener los valores deseados (1 y -1)
end


# ╔═╡ b2167e80-f1e4-4b65-bf09-707ad4a57f4d
unPaso() # Imprimimos la función 
...
# ╔═╡ a9fa23d0-4ad3-4ca5-90c9-d13822f6f2e6
function variosPasos(n) #Definimos la función 
	
    return 2 .* Int.(rand(Bool, n)) .- 1

	# Usamos rand(Bool, n ) para generar  valores booleanos aleatorios, y luego se convierte en un entero utilizando Int Luego, se multiplica por 2 y se resta 1 para obtener los valores deseados (1 y -1)
end


# ╔═╡ df724a4e-57d6-4941-9e55-79543142da54
variosPasos(8) #Imprimimos la función 

# ╔═╡ 5ebb82e0-67d9-4201-8ac7-1673eb21aedd
function caminataAleatoria(n)
    # Generar los pasos aleatorios utilizando la función variosPasos
    pasos = variosPasos(n)
    
    # Inicializar la posición actual en 0 y crear un arreglo para almacenar la caminata
    posicion_actual = 0
    caminata = [posicion_actual]
    
    # Iterar sobre cada paso y actualizar la posición actual
    for paso in pasos
        posicion_actual += paso
        push!(caminata, posicion_actual)  # Agregar la posición actual al arreglo caminata
    end
...
# ╔═╡ cd8367d3-ed1d-4ed8-be35-df2055cb1990
 caminataAleatoria(12) # Imprimimos la función
...
function graficaCaminata(caminata) # Definimos la función 
    num_pasos = length(caminata) # Obtenemos la longitud del arreglo 
    pasos = 0:num_pasos-1  # Generarmos un arreglo con el número de pasos ( va de 0 hasta num_pasos-1)

	#Creamos la gráfica
    scatter(pasos, caminata, xlabel = "Número de pasos", ylabel = "Posición", legend = false)
end


# ╔═╡ 4ad65879-e3c0-4d25-98ff-b617a3b845a2
begin
	n2 = 1000  # Definimos el valor de n2 como un número entero positivo
	
	caminata2 = caminataAleatoria(n2)  # Generamos una caminata aleatoria utilizando la función caminataAleatoria
	
	graficaCaminata(caminata2)  # Aplicamos la función graficaCaminata a la caminata aleatoria generada
...
# ╔═╡ 28c7b82d-49ab-44f5-9430-8cbf578163f3
begin
	function graficaCaminata1!(caminata) #Definimos la función 
    posiciones_x = caminata[1]  # Obtenemos las posiciones horizontales
    posiciones_y = caminata[2]  # Obtenemos las posiciones verticales
    
    plot!(posiciones_x, posiciones_y, linestyle=:dash, marker=:circle, markersize=3) #Creamos la gráfica
end

	# Generamos una gráfica con 5 caminatas aleatorias
	function generarGraficaCaminatas1()
	    n_caminatas = 5
	
	    for i in 1:n_caminatas
	        caminata_2d = caminataAleatoria2D(20)  # Generamos la caminata aleatoria en 2D
	        
	        if i == 1 #Hacemo un ciclo if definiendo las caracteristicas de la grafica 
	            plot(caminata_2d[1], caminata_2d[2], linestyle=:dash, marker=:circle, markersize=3, xlabel="Posición horizontal", ylabel="Posición vertical", legend=false)
	        else
	            graficaCaminata!(caminata_2d)
	        end
	    end
	end
	
	# Llamamos a la función para generar la gráfica
	generarGraficaCaminatas1()
...
# ╔═╡ 86c6e123-879f-408d-bacb-f449ae99ffe1

function animaCaminata(caminata) #Definimos la función
    num_pasos = length(caminata)  # Obtenemos la longitud del arreglo
    pasos = 0:num_pasos-1  # Generamos un rango de pasos
    
    
    anim = @animate for i in 1:num_pasos # Creamos la animación 
        graficaCaminata(caminata[1:i])  # Generamos la gráfica de la caminata hasta el paso actual i
    end
...
# ╔═╡ 0a8b068c-14ba-4624-ac25-c6ce90c2a443
begin
	caminata4= caminataAleatoria(1000)  # Generamos una caminata aleatoria
	anim = animaCaminata(caminata4)  # Creamos la animación de la caminata
	gif(anim, "caminata.gif")  # Guardamos la animación como un archivo GIF
...
function caminataAleatoria2DContinua(s) # Definimos la función 
    posiciones_x = zeros(s+1)  # Arreglo para las posiciones horizontales
    posiciones_y = zeros(s+1)  # Arreglo para las posiciones verticales
    
    for i in 2:s+1
        paso_x = aleatorioUniforme(-1, 1)  # Generamos un paso aleatorio horizontal
        paso_y = aleatorioUniforme(-1, 1)  # Generamos un paso aleatorio vertical
        
        posiciones_x[i] = posiciones_x[i-1] + paso_x  # Calculamos la nueva posición horizontal
        posiciones_y[i] = posiciones_y[i-1] + paso_y  # Calculamos la nueva posición vertical
    end
    
    return [posiciones_x, posiciones_y]  # Devolvemos el arreglo con las posiciones horizontales y verticales
end


# ╔═╡ 9f7eb66f-b17d-44ee-b60f-3c3de805fca8
begin
	s = 1# Número de pasos
	caminata_continua = caminataAleatoria2DContinua(s)  # Generaamos la caminata aleatoria
...
# ╔═╡ ba0984f0-6b7c-4e17-87b0-9fbd6e250d76
...
# ╔═╡ 9fa33d9f-1486-4b17-b200-fcbe8323c028

function animaCaminata2D(caminata)
    # Creamos la animación utilizando la macro @animate de la biblioteca Plots
    anim = @animate for i in 1:length(caminata[1])
        # En cada iteración, se trazan los puntos de la caminata hasta el índice correspondiente
        plot(caminata[1][1:i], caminata[2][1:i], xlims=(-4, 4), ylims=(-4, 4), aspect_ratio=1, legend=false)
    end
    # Guardarmos la animación en un archivo GIF 
    gif(anim, "caminata.gif", fps=10)
end


# ╔═╡ 47f2afee-d935-41ab-96b7-7527f675aa8c
begin
	caminata = [[-3, -2, 2, 3, 4, 5], [-3, -1, 0, 2, 0, -3]] # Colocamos los puntos/arreglos
	animaCaminata2D(caminata) # Guardamos la animación como un archivo GIF
...
begin
	...
	num_steps = 500  # Número de pasos en la caminata
	step_length = 1  # Longitud de cada paso
	
	# Generamos los desplazamientos en x, y, z de forma aleatoria
	random_displacements = step_length * randn(num_steps, 3)
	
	# Calculamos las posiciones acumuladas en x, y, z
	positions = cumsum(random_displacements, dims=1)
	
	# Graficamos la caminata aleatoria
	plot3d(positions[:, 1], positions[:, 2], positions[:, 3], xlabel="X", ylabel="Y", zlabel="Z", legend=false, title= "Caminata aleatoria en tres dimensiones")
...
# ╟─1e68a5ca-d78e-4371-a63c-d329d11b187f
# ╠═1ed85ab5-476d-4851-a0ab-c627f972b2c9
# ╠═26ba80ed-6766-4191-8350-4f29070c75c9
# ╠═4ad65879-e3c0-4d25-98ff-b617a3b845a2
# ╟─07edca52-f4bc-478f-b823-9811c320e171
# ╠═28c7b82d-49ab-44f5-9430-8cbf578163f3
# ╟─75d3a337-a3b2-4319-bd1b-72d2642caef7
